...
export class GeneExpressionComparisonComponent implements OnInit {
  ...
  getTableSize() {
    ...
    let table = 'g2g'
    let apiUrl = "//seahorse-api.tm4.org:8001/gtex.json?";
    let annotationUrl = `sql=SELECT%0D%0A++COUNT%28*%29%0D%0AFROM%0D%0A++${table}%0D%0Awhere%0D%0A++GeneA+is+"${this.searchValue}"`
    let queryURL = `${apiUrl}${annotationUrl}`;
    this.httpClient.get(queryURL).pipe(
      catchError(error => {
        console.log("Error: ", error);
      ...
      .subscribe(res => {
        // console.log("table size: ", res['rows'][0][0], this.searchValue)
  ...
  getComparisonStats() {
    let apiUrl = "//seahorse-api.tm4.org:8001/gtex.json?";
    let annotationUrl = `sql=select%0D%0A++distinct+g2g.GeneB%2C%0D%0A++e2s.SYMBOL%2C%0D%0A++e2s.ENTREZID%0D%0Afrom%0D%0A++g2g%0D%0A++join+e2s+on+g2g.GeneB+%3D+e2s.ENSEMBL%0D%0Awhere%0D%0A++g2g.GeneA+is+"${this.searchValue}"%0D%0Alimit%0D%0A++${this.currPage}%2C+${this.limit}`
    let queryURL = `${apiUrl}${annotationUrl}`;
    this.httpClient.get(queryURL).pipe(
      catchError(error => {
        console.log("Error: ", error);
  ...
  geneSearch() {
    ...
    this.tableSize = 0;
    // console.log("gene search started.", this.searchValue)
    this.getTableSize()
    this.getComparisonStats()
